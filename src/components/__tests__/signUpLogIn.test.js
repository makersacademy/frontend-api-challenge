...
describe('signing up', () => {
  test('rejects a name that is in use when signing up', async () => {
    fetch.mockResponseOnce(JSON.stringify({ handle: ['has already been taken'] }));
    render(<Router><SignUp /></Router>);

    const nameField = screen.getByLabelText('Name:');
    fireEvent.change(nameField, { target: { value: 'Phil' } });
    fireEvent.click(screen.getByText('Sign up'));
    await waitFor(() => expect(screen.getByText('Sorry, that name has already been taken, please choose another')));
  });
