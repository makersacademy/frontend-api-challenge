...
describe("ChitterView", () => {
  beforeEach(() => {
    ...
    api.fetchPeeps.mockImplementation(() => {
      return [
        {
          body: "this is a peep from the mock api",
          created_at: "2022-06-04T17:14:37.409Z",
          user: { handle: "luke" },
        },
      ];
    });
  ...
  describe("displayPeepsFromApi", () => {
    it("should display peeps from server on page", async () => {
      ...
      expect(document.querySelectorAll("li.peep").length).toBe(1);
      expect(document.querySelectorAll("li.peep p.peep")[0].innerHTML).toEqual(
  ...
  describe("createAccount", () => {
    it("signs up the user", async () => {
      ...
      expect(document.querySelector("#success-alert").innerText).toEqual(
  ...
  describe("newPeep", () => {
    it("should create a new peep", async () => {
      ...
      expect(document.querySelectorAll("li.peep").length).toBe(1);
      expect(document.querySelectorAll("li.peep p.peep")[0].innerHTML).toBe(
        "my first peep"
      );
    });
